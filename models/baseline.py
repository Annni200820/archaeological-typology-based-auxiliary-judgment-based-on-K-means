...
def visualize_clusters(image_feature_embedding, kmeans_labels, image_paths, cluster_centers):
    ...
    for label in set(kmeans_labels):
        ...
        color = plt.cm.get_cmap(cmap_name)(label / num_clusters)  # get the color based on the cluster labels
    ...
    plt.scatter(cluster_centers[:, 0], cluster_centers[:, 1], marker='o', c='r', s=100)
    ...
    for center in cluster_centers:
        ...
        # plot circle
        circle = patches.Circle(circle_center, circle_radius, edgecolor='r', facecolor='none', linewidth=1)
...
def cluster_images(image_feature_list, image_path_list):
    ...
    for num_clusters in range(3, max_clusters + 1):
        kmeans = sklearn.cluster.KMeans(n_clusters=num_clusters, random_state=0).fit(np.array(img_feature_embedding))
    ...
    kmeans = sklearn.cluster.KMeans(n_clusters=optimal_num_clusters, random_state=0).fit(np.array(img_feature_embedding))
    ...
    save_path = 'E:\jade_typological_judge\jade_cluster'    # change it to your own path
    ...
    f, axarr = plt.subplots(optimal_num_clusters, 5)
